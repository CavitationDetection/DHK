...
@register_model
def uniformer_small(num_classes: int, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
...
@register_model
def uniformer_small_plus(num_classes: int, pretrained=True, **kwargs):
    model = UniFormer(
...
@register_model
def uniformer_small_plus_dim64(num_classes: int, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 5, 9, 3], conv_stem=True,
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_classes = num_classes, **kwargs)
...
@register_model
def uniformer_base(num_classes: int, pretrained=True, **kwargs):
    model = UniFormer(
        depth=[5, 8, 20, 7],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_classes = num_classes, **kwargs)
...
@register_model
def uniformer_base_ls(num_classes: int, pretrained=True, **kwargs):
    global layer_scale
    layer_scale = True
    model = UniFormer(
        depth=[5, 8, 20, 7],
